(()=>{
'use strict';
if(window.__FIXEDCOIN_FORGE_COLLISION_V5__) return;
window.__FIXEDCOIN_FORGE_COLLISION_V5__=true;
...
/* One particle is a small comet, not a bare dot. We intentionally keep a
   staggered queue so the impacts remain readable and the neon can alternate
   OFF -> ON -> OFF for every arrival. */
...
function pulseCounter(kind){
  ...
  const step=()=>{
    ...
    card.style.setProperty('--collision-alpha',(0.92+Math.random()*0.08).toFixed(3));
...
function impactRing(kind,x,y){
  ...
  const anim=ring.animate([
    ...
    {transform:'translate(-50%,-50%) scale(1.35)',opacity:.72,offset:.28},
...
function addEventParticle(kind,index,total){
  ...
  el.style.setProperty('--tail-size',`${10+Math.random()*12}px`);
  layer.appendChild(el);
  const duration=(kind==='accept'?2350:2200)*(0.94+Math.random()*.12);
  const delay=index*420+Math.random()*90;
