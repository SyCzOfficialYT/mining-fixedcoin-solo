...
const fmtTime=v=>{const d=v?new Date((Number(v)>1e12?Number(v):Number(v)*1000)):new Date();return Number.isFinite(d.getTime())?d.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'}):'—'};
async function syncReferenceTelemetry(){try{const r=await fetch('/api/status?reference='+Date.now(),{cache:'no-store'});if(!r.ok)return;const s=await r.json(),w=s.wallet||{},m=s.mining||{},n=s.node||{},round=s.round||{},server=s.server||{},shares=Array.isArray(s.shares)?s.shares:[],now=Date.now()/1000,recent=shares.filter(x=>Number(x?.epoch)>0&&now-Number(x.epoch)>=0&&now-Number(x.epoch)<=600&&Number(x?.work||0)>0);const avg=recent.length?recent.reduce((a,x)=>a+Number(x.work||0),0)/recent.length:0;const oldest=recent.length?Number(recent[0]?.epoch||0):0;const newest=recent.length?Number(recent.at(-1)?.epoch||0):0;const sharesPerMin=(newest>oldest)?recent.length/(Math.max(1,newest-oldest)/60):0;const hs=Number(m.hashrate_5m)||0,diff=Number(n.difficulty||round.difficulty||0),eta=hs>0&&diff>0?diff*4294967296/hs:0;const workers=Object.keys(m.workers||{}).length||Number(m.worker_count)||0;const luck=Number(round.luck_pct??round.network_luck??100);const estimated=w.estimated_earnings??w.estimated??0;const pending=w.unconfirmed??w.pending??0;const paid=w.total_rewards??w.total_paid??0;const vals={confirmedBalance:fmtCoin(w.confirmed),unconfirmedBalance:fmtCoin(w.unconfirmed),immatureBalance:fmtCoin(w.immature),totalBalance:fmtCoin(w.total),liveVarDiff:fmtDiff(Object.values(m.workers||{}).reduce((d,x)=>Math.max(d,Number(x?.difficulty)||0),Number(m.fixed_difficulty)||0)),avgDiff:fmtDiff(avg),sharesMin:sharesPerMin.toFixed(1),eta:eta?fmtDuration(eta):'—',estimatedEarnings:fmtCoin(estimated),pendingPayout:fmtCoin(pending),totalPaid:fmtCoin(paid),railWorkers:String(workers),liveWorkers:String(workers),networkLuck:Number.isFinite(luck)?luck.toFixed(1)+'%':'100.0%',networkUptime:server.uptime||fmtDuration(Number(s.uptime_seconds)||0),serverTime:server.time||fmtTime(s.ts||Date.now()),uptimeRail:fmtDuration(Number(s.uptime_seconds)||0),balanceUsd:'≈ '+(Number(w.usd_value??w.total_usd??0)||0).toFixed(2)+' USD',roundAge:round.started_at?fmtDuration(Math.max(0,Date.now()/1000-(Date.parse(String(round.started_at).replace(' ','T'))/1000))):'—'};Object.entries(vals).forEach(([id,value])=>{const el=$(id);if(el)el.textContent=value})}catch(_){} }
...
function paintParallax(){raf=0;surfaces.forEach((el,i)=>{const depth=(i%2?1:.82);el.style.setProperty('--rf-x',(px*depth).toFixed(2)+'deg');el.style.setProperty('--rf-y',(py*depth).toFixed(2)+'deg');el.style.transform=`perspective(900px) rotateY(calc(3deg + var(--rf-x,0deg))) rotateX(calc(2deg + var(--rf-y,0deg))) translateZ(${Math.abs(px+py)*.12}px)`});if(stage){stage.style.setProperty('--scene-x',(px*.55).toFixed(2)+'px');stage.style.setProperty('--scene-y',(py*.55).toFixed(2)+'px')}}
...
const realtimeStyle=document.createElement('style');realtimeStyle.textContent=`
...
.reference-dashboard .share-burst{position:absolute;left:50%;top:50%;z-index:26;pointer-events:none;color:var(--green);font:800 10px/1 var(--mono);letter-spacing:.16em;text-shadow:0 0 10px rgba(77,255,114,.9);transform:translate(-50%,-50%);animation:rfShareBurst .78s cubic-bezier(.16,.84,.22,1) forwards}
.reference-dashboard .share-burst.reject{color:var(--red);text-shadow:0 0 10px rgba(255,79,97,.9)}
.reference-dashboard .core-hot{filter:drop-shadow(0 0 26px rgba(112,236,255,.55)) drop-shadow(0 0 38px rgba(165,92,255,.45))}
@keyframes rfShareCard{0%{transform:perspective(900px) rotateY(3deg) rotateX(2deg) translateZ(0) scale(1)}24%{transform:perspective(900px) rotateY(3deg) rotateX(2deg) translateZ(18px) scale(1.045);box-shadow:inset 0 0 0 1px rgba(77,255,114,.2),0 0 32px rgba(77,255,114,.38),0 20px 45px rgba(0,0,0,.7)}100%{transform:perspective(900px) rotateY(3deg) rotateX(2deg) translateZ(0) scale(1)}}
