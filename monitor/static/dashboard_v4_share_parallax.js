(() => {
  'use strict';
  if (window.__FIXEDCOIN_SHARE_PARALLAX_V8__) return;
  window.__FIXEDCOIN_SHARE_PARALLAX_V8__ = true;

  const stage = document.getElementById('forgeStage');
  if (!stage) return;
  const cards = [
    stage.querySelector('.forge-counter.accepted'),
    stage.querySelector('.forge-counter.rejected'),
  ].filter(Boolean);
  if (!cards.length) return;

  let raf = 0, tx = 0, ty = 0, cx = 0, cy = 0;
  const clamp = (v, a, b) => Math.max(a, Math.min(b, v));

  const render = () => {
    raf = 0;
    cx += (tx - cx) * 0.12;
    cy += (ty - cy) * 0.12;
    const px = cx.toFixed(3) + 'deg';
    const py = cy.toFixed(3) + 'deg';
    for (const c of cards) {
      c.style.setProperty('--px', px);
      c.style.setProperty('--py', py);
    }
    if (Math.abs(tx - cx) > 0.015 || Math.abs(ty - cy) > 0.015)
      raf = requestAnimationFrame(render);
  };
  const go = () => { if (!raf) raf = requestAnimationFrame(render); };
  const set = (x, y) => { tx = clamp(x, -3, 3); ty = clamp(y, -2, 2); go(); };

  const pointer = (x, y) => {
    const r = stage.getBoundingClientRect();
    if (!r.width || !r.height) return;
    const nx = clamp(((x - r.left) / r.width) * 2 - 1, -1, 1);
    const ny = clamp(((y - r.top) / r.height) * 2 - 1, -1, 1);
    set(-nx * 3, -ny * 2);
  };

  stage.addEventListener('pointermove', e => pointer(e.clientX, e.clientY), { passive: true });
  stage.addEventListener('pointerleave', () => set(0, 0), { passive: true });
  stage.addEventListener('touchmove', e => { const t = e.touches[0]; if (t) pointer(t.clientX, t.clientY); }, { passive: true });
  stage.addEventListener('touchend', () => set(0, 0), { passive: true });
  window.addEventListener('blur', () => set(0, 0), { passive: true });
})();
