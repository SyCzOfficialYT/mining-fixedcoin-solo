(() => {
  'use strict';
  if (window.__FIXEDCOIN_SHARE_PARALLAX_V6__) return;
  window.__FIXEDCOIN_SHARE_PARALLAX_V6__ = true;
  ...
  const render = () => {
    ...
    for (const c of cards) {
      c.style.setProperty('--px', cx.toFixed(3) + 'deg');
      c.style.setProperty('--py', cy.toFixed(3) + 'deg');
    }
    if (Math.abs(tx - cx) > 0.01 || Math.abs(ty - cy) > 0.01)
      raf = requestAnimationFrame(render);
  };
  const go = () => { if (!raf) raf = requestAnimationFrame(render); };
  const set = (x, y) => { tx = clamp(x, -1, 1); ty = clamp(y, -0.6, 0.6); go(); };
