...
function renderBalance(s){
  ...
  // Dashboard semantics are intentional:
  // TOTAL follows CONFIRMED, while UNCONFIRMED follows IMMATURE.
  const total=confirmed;
  const unconfirmed=immature;

  setBalance('balanceConfirmed',confirmed);
  setBalance('balanceUnconfirmed',unconfirmed);
