(()=>{
'use strict';
if(window.__FIXEDCOIN_ANIMATION_PERF_V1__)return;
window.__FIXEDCOIN_ANIMATION_PERF_V1__=true;
...
const fx={canvas:null,ctx:null,pc:null,pctx:null,particles:[],events:[],progress:[],raf:0,last:0,visible:true,geom:{core:{x:0,y:0},accept:{x:0,y:0},reject:{x:0,y:0}},trackGeom:{x:0,y:0,w:0,h:0},progressValue:-1,lastA:Number(accepted?.textContent)||0,lastR:Number(rejected?.textContent)||0};
...
function syncProgress(){if(!track||!fx.pc)return;const p=clamp(parseFloat(String(pctEl?.textContent||'0').replace('%',''))||0,0,100);if(fx.progressValue<0)fx.progressValue=p;if(Math.abs(p-fx.progressValue)>.01){const delta=p-fx.progressValue;const n=Math.min(14,Math.max(2,Math.round(Math.abs(delta)*.7)));for(let i=0;i<n;i++)fx.progress.push({x:0.06+Math.min(.88,p/100)*.88+(Math.random()-.5)*.035,y:.5+(Math.random()-.5)*.32,v:.0015+Math.random()*.0028,life:0,max:.35+Math.random()*.5,s:1+Math.random()*2,a:.25+Math.random()*.55});fx.progressValue=p}}
