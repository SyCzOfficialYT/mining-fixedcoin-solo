...
def wallet_state(current_height):
    balances, balances_error = rpc("getbalances", timeout=4); walletinfo, _ = rpc("getwalletinfo", timeout=4); txs, tx_error = rpc("listtransactions", ["*",200,0,True], timeout=5)
    mine=(balances or {}).get("mine") or {}; confirmed=as_number(mine.get("trusted")); pending=as_number(mine.get("untrusted_pending")); immature=as_number(mine.get("immature")); blocks=[]; seen=set()
    for tx in txs or []:
        ...
        seen.add(txid); height=int(tx.get("blockheight") or tx.get("height") or 0); confirmations=int(tx.get("confirmations") or 0)
        if height<=0 and confirmations>0 and current_height: height=max(0,current_height-confirmations+1)
        reward=abs(as_number(tx.get("amount"))); maturity_height=height+MATURITY if height else 0; remaining=max(0,maturity_height-current_height) if maturity_height and current_height else None
