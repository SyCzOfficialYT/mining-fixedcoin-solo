...
def parse_logs():
    ...
    for line in lines(LOG):
        ...
        if m:
            worker = m.group(1)
        m = re.search(
            r"Job\s+id=([^\s]+).*?(?:height=)?(\d+).*?(?:net_diff|network_diff)[≈:=~]?\s*([0-9.eE+-]+)",
            line,
            re.I,
        )
        if m:
        ...
        m = re.search(
            r"ACCEPT\s+#(\d+)\s+work=([0-9.]+).*?(?:pool|pool_diff)=([0-9.]+).*?hash=([0-9a-fA-F]+)",
            line,
            re.I,
        )
        if m:
        ...
        if re.search(r"\bREJECT\b", line, re.I):
            rejected += 1
        m = re.search(
            r"BLOCK ACCEPTED.*?height=(\d+).*?hash=([0-9a-fA-F]{16,64})(?:.*?reward=([0-9.]+))?",
            line,
            re.I,
        )
        if m:
...
def status():
    ...
    share_target = None
    if network_target_int and network_diff > 0 and fixed_difficulty > 0:
        share_target = target_hex(int(network_target_int * network_diff / fixed_difficulty))

    mine = balances.get("mine") or {}
    trusted = float(mine.get("trusted") or 0)
    pending = float(mine.get("untrusted_pending") or 0)
    ...
    for share in shares:
        w = workers.setdefault(
            share["worker"],
            {"accepted": 0, "rejected": 0, "difficulty": share["pool_diff"]},
        )
        w["accepted"] += 1
    ...
    return {
        "node": {
            "online": bool(info),
            "synced": bool(info) and not info.get("initialblockdownload", False),
            "initial_block_download": bool(info.get("initialblockdownload", False)),
            "height": height,
            "headers": headers,
            ...
            "verification_progress": float(info.get("verificationprogress") or 0),
        },
        "next": {
            "height": next_height,
        ...
        "competition": {
            ...
            "network_share_ppm": competition * 10000,
        },
        "mining": {
            "accepted": len(shares),
            "rejected": rejected,
            "reject_pct": round(100 * rejected / max(1, len(shares) + rejected), 2),
            "hashrate": h5,
            "hashrate_1h": h1,
        ...
        "wallet": {
            ...
            "pending": pending,
            "immature": immature,
            "total_rewards": sum(x["reward"] for x in blocks),
        },
        "blocks": blocks,
        "shares": shares,
        "job": parsed_job,
...
def api_logs():
    ...
    for line in lines(LOG, 180):
        if any(k in line for k in ("ACCEPT", "REJECT", "BLOCK", "ERROR", "NEW ROUND", "authorize", "Job id=")):
            level = (
                "success" if "ACCEPT" in line or "BLOCK" in line
                else "danger" if "ERROR" in line
                else "warning" if "REJECT" in line
                else "info"
            )
