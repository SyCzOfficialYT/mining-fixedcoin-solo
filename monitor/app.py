...
def parse_logs():
    ...
    for line in lines(LOG):
        ...
        if m:
            name,diff=m.groups(); current_worker=name; w=workers.setdefault(name,{"accepted":0,"rejected":0,"difficulty":float(diff)}); w["difficulty"]=float(diff); w["last_seen"]=parse_ts(line[:19])
        ...
        if m:
            num,work,diff,h=m.groups(); worker=current_worker or "unknown"; accepted.append({"ts":line[:19],"epoch":parse_ts(line),"num":int(num),"work":float(work),"pool_diff":float(diff),"hash":h[:16],"worker":worker}); w=workers.setdefault(worker,{"accepted":0,"rejected":0,"difficulty":float(diff)}); w["accepted"]+=1; w["last_seen"]=parse_ts(line)
...
def wallet_state(current_height):
    balances, balances_error=rpc("getbalances",timeout=4); walletinfo,_=rpc("getwalletinfo",timeout=4)
    txs,tx_error=rpc("listtransactions",["*",200,0,True],timeout=5)
    mine=(balances or {}).get("mine") or {}; confirmed=as_number(mine.get("trusted")); rpc_pending=as_number(mine.get("untrusted_pending")); rpc_immature=as_number(mine.get("immature"))
    ledger=read_ledger(); blocks=[]; seen=set()
    for b in ledger:
        if not isinstance(b,dict): continue
        txid=str(b.get("txid") or ""); bh=int(b.get("height") or 0)
        key=(txid,bh)
        if not txid and not bh or key in seen: continue
        seen.add(key)
        confirmations=max(0,int(b.get("confirmations") or (current_height-bh+1 if bh else 0)))
        maturity_height=int(b.get("maturity_height") or (bh+MATURITY if bh else 0)); remaining=max(0,maturity_height-current_height) if maturity_height else MATURITY
        ...
        blocks.append({"txid":txid,"height":bh,"confirmations":confirmations,"reward":abs(as_number(b.get("reward"))),"category":"generate","state":state,"maturity_height":maturity_height,"maturity_remaining":remaining,"time":parse_ts(b.get("found_at") or b.get("last_seen")) if isinstance(b.get("found_at") or b.get("last_seen"),str) else int(b.get("time") or 0),"blockhash":str(b.get("blockhash") or ""),"status":state})
    # Wallet RPC remains authoritative for balances. A coinbase reward is technically
    # IMMATURE until maturity, so expose that amount explicitly while retaining the
    # normal untrusted-pending field for ordinary unconfirmed transactions.
    ...
    return {"confirmed":confirmed,"pending":rpc_pending,"immature":immature,"total":confirmed+rpc_pending+immature,"blocks":blocks[:100],"wallet":walletinfo or {},"error":balances_error or tx_error}
...
def status():
    ...
    job={"job_id":log_job.get("job_id"),"height":stats.get("round_height") or log_job.get("height") or height,"network_diff":network_diff}; job.update(log_job); round_best=as_number(stats.get("round_best"),max((as_number(x.get("work")) for x in shares),default=0)); round_work=as_number(stats.get("round_work")); round_shares=int(stats.get("round_shares") or 0); best_pct=(round_best/network_diff*100) if network_diff else 0; remaining=max(0,network_diff-round_best) if network_diff else 0; round_effort=as_number(stats.get("round_effort_pct"),best_pct); h5=hashrate(shares,300); h1=hashrate(shares,3600); network_hashrate=as_number(mininginfo.get("networkhashps"))
    ...
    for name in set(log_workers)|set(persisted):
        lv=log_workers.get(name,{}); pv=persisted.get(name,{}) if isinstance(persisted.get(name,{}),dict) else {}; workers[name]={"accepted":int(pv.get("ok") or pv.get("accepted") or pv.get("shares") or lv.get("accepted") or 0),"rejected":int(pv.get("bad") or pv.get("rejected") or lv.get("rejected") or 0),"difficulty":as_number(pv.get("difficulty") or lv.get("difficulty") or fixed_diff,fixed_diff),"active":bool(lv.get("active"))}
