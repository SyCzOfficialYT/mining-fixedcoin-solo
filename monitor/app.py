...
DATA=Path('/app/data'); RPC_URL=f"http://127.0.0.1:{os.getenv('FIX_RPCPORT','24761')}"; RPC_AUTH=HTTPBasicAuth(os.getenv('FIX_RPCUSER','fixrpc'),os.getenv('FIX_RPCPASS','')); PAYOUT=os.getenv('FIX_PAYOUT_ADDRESS','fix1CHANGE_ME'); MATURITY=int(os.getenv('COINBASE_MATURITY','100')); LOG=DATA/'stratum.log'
...
def parse_logs():
    accepted=[]; rejected=0; blocks=[]; worker='unknown'
...
def status():
    info,_=rpc('getblockchaininfo');net,_=rpc('getnetworkinfo');bal,_=rpc('getbalances');shares,rejected,blocks=parse_logs();height=int((info or {}).get('blocks') or 0);network_diff=float((info or {}).get('difficulty') or 0);mine=(bal or {}).get('mine') or {};trusted=float(mine.get('trusted') or 0);pending=float(mine.get('untrusted_pending') or 0);immature=float(mine.get('immature') or 0);best=max((x['work'] for x in shares),default=0);last=shares[-1]['work'] if shares else 0;effort=min(100,100*best/network_diff) if network_diff else 0;span=max(1,len(shares)*2);hashrate=sum(x['pool_diff'] for x in shares)*(2**32)/span if shares else 0
    workers={}
    for x in shares:
