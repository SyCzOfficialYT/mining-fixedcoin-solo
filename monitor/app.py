...
def hashrate(shares, window):
    """Stable share-work estimate over a fixed observation window.

    Do not divide by the time between the first and last share. During startup
    that turns a few lucky shares into absurd multi-terahash readings. The
    denominator is the requested 5m/1h window, so the value converges naturally.
    """
    now = time.time()
    ...
    return expected_hashes / float(window)
...
def normalize_recent(raw):
    ...
    for x in raw[-200:]:
        ...
        epoch=parse_ts(ts) if isinstance(ts,str) else float(ts or 0)
...
def status():
    stats=read_stats(); log_shares,log_rejected,log_blocks,log_workers,log_job=parse_logs()
    shares=normalize_recent(stats.get("recent_shares")) or log_shares
    rejected=int(stats.get("shares_bad") or log_rejected or 0); accepted=int(stats.get("shares_ok") or len(shares))
    info,info_error=rpc("getblockchaininfo"); net,_=rpc("getnetworkinfo"); mininginfo,_=rpc("getmininginfo"); balances,_=rpc("getbalances")
    info=info or {}; net=net or {}; mininginfo=mininginfo or {}; balances=balances or {}
    pool=config(); fixed_diff=as_number(pool.get("fixed_difficulty",13354),13354)
    network_diff=as_number(stats.get("network_diff"),0) or as_number(log_job.get("network_diff"),0) or as_number(mininginfo.get("difficulty"),0)
    height=int(stats.get("round_height") or info.get("blocks") or mininginfo.get("blocks") or log_job.get("height") or 0)
    headers=int(info.get("headers") or height); initial_sync=bool(info.get("initialblockdownload",False))
    ...
    round_work=as_number(stats.get("round_work"),0); round_shares=int(stats.get("round_shares") or 0)
    best_pct=(round_best/network_diff*100) if network_diff else 0.0
    remaining=max(0.0,network_diff-round_best) if network_diff else 0.0
    round_effort=as_number(stats.get("round_effort_pct"),best_pct)
    h5=hashrate(shares,300); h1=hashrate(shares,3600); network_hashrate=as_number(mininginfo.get("networkhashps"),0)
    competition=h5/network_hashrate*100 if network_hashrate else 0
    ...
    mine=balances.get("mine") or {}; confirmed=as_number(mine.get("trusted"),0); pending=as_number(mine.get("untrusted_pending"),0); immature=as_number(mine.get("immature"),0)
    ...
    if not blocks: blocks=log_blocks
    return {
      "status":"online" if info or stats else "degraded","last_update":int(time.time()),
      "node":{"online":bool(info) or bool(stats),"synced":bool(info) and not initial_sync,"initial_block_download":initial_sync,"height":height,"headers":headers,"difficulty":network_diff,"target":mininginfo.get("target"),"bits":mininginfo.get("bits"),"connections":int(net.get("connections") or 0),"network_hashrate":network_hashrate,"chain":info.get("chain") or mininginfo.get("chain") or "unknown","verification_progress":as_number(info.get("verificationprogress"),0),"rpc_error":info_error},
      "mining":{"accepted":accepted,"rejected":rejected,"reject_pct":round(100*rejected/max(1,accepted+rejected),3),"hashrate_5m":h5,"hashrate_1h":h1,"fixed_difficulty":fixed_diff,"best_share":round_best,"best_share_pct":best_pct,"difficulty_remaining":remaining,"round_work":round_work,"round_shares":round_shares,"round_effort":round_effort,"workers":workers,"worker_count":len(active_workers),"active_workers":active_workers},
      "round":{"height":int(stats.get("round_height") or height),"shares":round_shares,"work":round_work,"best_share":round_best,"effort_pct":round_effort,"best_share_pct":best_pct,"difficulty":network_diff,"remaining":remaining,"started_at":stats.get("round_started_at")},
      ...
      "job":job,"shares":shares[-100:],"blocks":blocks[-100:],"history_diff":DIFF_HISTORY,"payout":pool.get("payout_address",""),"maturity":MATURITY,"ts":int(time.time())
    }
