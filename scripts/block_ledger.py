...
def rpc(method, params=None):
    """Use the exact authenticated RPC configuration generated by entrypoint.sh."""
    import requests
    ...
    for height in sorted(heights):
        ...
        if row is None:
            ...
            print(
                ...
                f"reward={found['reward']:.8f}", flush=True,
            )
        else:
            row["last_seen"] = now()
...
def main():
    ...
    # Fail loudly on authentication/RPC problems instead of silently leaving
    # an empty persistent ledger while the dashboard continues to run.
    try:
