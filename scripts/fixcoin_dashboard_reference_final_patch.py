...
VISUAL_CSS.write_text(
    css + '\n/* monitor runtime safety */\n.reference-dashboard img{max-width:100%;}\n',
    encoding='utf-8',
)

# Make the final reference runtime an explicit part of the production page.
# This is intentionally injected here rather than relying on another visual
# patch to load it, so the canonical reference layer cannot silently disappear.
ref_script = '<script src="/static/dashboard_v4_reference_final.js?v=20260828-reference-final"></script>'
if '/static/dashboard_v4_reference_final.js' not in html:
    if '</body>' not in html:
        raise RuntimeError('dashboard reference final: template body anchor missing')
    html = html.replace('</body>', ref_script + '\n</body>', 1)
...
# Validate stable semantic reference anchors. Do not compare the complete
# class attribute literally: reference markup legitimately combines classes
# (for example `forgeGrid forgeStage`) and JS-generated classes use kebab-case.
required_html = [
    'class="liveshareApp reference-dashboard"',
    'class="leftRail"',
    'class="rightRail"',
    'class="heroPanel ornatePanel"',
    ...
    'id="blockHistoryList"',
    'id="historyCount"',
    'dashboard_v4_reference_visual.css',
    '/static/dashboard_v4_reference_final.js',
]
missing = [x for x in required_html if x not in html]

# These are actual semantic selectors used by the reference stylesheet.
# Renderer-only class names are deliberately not validated here.
for token in (
    '.liveshareApp',
    '.heroPanel',
    '.dragon',
...
# Responsive CSS is required, but tolerate normal whitespace formatting.
if not re.search(r'@media\s*\(max-width\s*:\s*(900|620)px\)', css):
    missing.append('reference css responsive breakpoints')
...
if duplicates:
    raise RuntimeError(
        'dashboard reference final: duplicate HTML id detected: ' + ', '.join(duplicates)
    )

if missing:
    raise RuntimeError(
        'dashboard reference final validation failed: ' + ', '.join(missing)
    )

print(
    'dashboard reference final verified: canonical Arcane LiveShare composition, '
    'dual dragons, crystal forge, ornate panels, balance scale, right telemetry rail, '
    'Chronicle history, explicit reference runtime, and responsive desktop/mobile layout'
)
