...
FINAL = f'<link rel="stylesheet" href="/static/dashboard_v4_forge_final.css?v={VERSION}">'
...
for pattern in (
    ...
    r'<link rel="stylesheet" href="/static/dashboard_v4_forge_final\.css\?v=[^"]+">',
...
if forge_end < 0: raise RuntimeError('dashboard forge closing section missing')
html = html[:forge_end] + combo_html + candidate_html + html[forge_end:]

required = ['class="forge panel"','id="forgeStage"','id="forgeCore"','id="candidate"','id="candidatePct"','class="candidate-track"','id="combo"','dashboard_v4_forge_reference.css','dashboard_v4_forge_upgrade.css','dashboard_v4_forge_transparency.css','dashboard_v4_forge_final.css','dashboard_v4_forge_upgrade.js']
...
print('dashboard forge upgrade applied: muted idle share cards, event-only neon, center-only persistent progress particles, transparent candidate HUD')
