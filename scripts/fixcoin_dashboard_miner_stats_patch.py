#!/usr/bin/env python3
"""Add a real per-miner statistics view to LiveShare.

The dashboard derives worker metrics from the Stratum recent_shares stream and
persisted worker counters. No fake miner telemetry is introduced.
"""
...
if "def build_miner_stats(" not in app:
    ...
    helper = r'''def build_miner_stats(shares, persisted_workers, parsed_workers, now=None):
    ...
    for worker, row in by_worker.items():
        ...
        miners.append({
            ...
            "rejected": rejected,
            "total_shares": total,
            "reject_pct": round(100.0 * rejected / max(1, total), 3),
            "hashrate_5m": h5,
            "hashrate_1m": h1,
            "difficulty": difficulty,
            "best_share": best,
            "last_share": latest.get("ts") or "—",
            "last_seen": last_epoch,
            "active": bool(last_epoch and now - last_epoch <= WORKER_ACTIVE_SECONDS),
...
replacement = needle + '    miner_stats=build_miner_stats(shares, persisted, log_workers, time.time())\n'
if 'miner_stats=build_miner_stats(' not in app:
    app = app.replace(needle, replacement, 1)

needle2 = '"active_workers":active_workers},"round":'
if needle2 not in app:
    raise RuntimeError("mining response marker not found in monitor/app.py")
app = app.replace(needle2, '"active_workers":active_workers},"miners":miner_stats,"round":', 1)
APP.write_text(app)

html = HTML.read_text()
if 'dashboard_miner_stats.css' not in html:
    html = html.replace('</head>', '<link rel="stylesheet" href="/static/dashboard_miner_stats.css?v=20260826-miners1">\n</head>', 1)

section = r'''<section class="miners panel" id="minersPanel">
  <div class="miners-head">
    <div class="section-heading"><span class="target-icon">♜</span><div><h2>MINER ARMORY // LIVE WORKERS</h2><p>Per-miner telemetry from the Stratum share stream.</p></div></div>
    <div class="miners-tools"><span id="minerCountBadge">0 ONLINE</span><select id="minerSelect" aria-label="Select miner"><option value="__all__">ALL MINERS</option></select></div>
  </div>
  <div class="miner-grid" id="minerGrid"><div class="miner-empty">Waiting for miner telemetry…</div></div>
</section>
'''
...
if 'dashboard_miner_stats.js' not in html:
    html = html.replace('<script defer src="/static/dashboard_v4_eta_plus.js?v=20260826-mythic1"></script>', '<script defer src="/static/dashboard_v4_eta_plus.js?v=20260826-mythic1"></script><script defer src="/static/dashboard_miner_stats.js?v=20260826-miners1"></script>', 1)
HTML.write_text(html)

if not CSS.exists():
    CSS.write_text(r'''.miners{margin-top:10px;padding:20px 22px;position:relative;overflow:hidden}.miners:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 80% 0%,rgba(155,92,255,.10),transparent 35%);pointer-events:none}.miners-head{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:16px}.miners-tools{display:flex;align-items:center;gap:10px}.miners-tools span{font:700 10px/1 JetBrains Mono,monospace;letter-spacing:.12em;color:#59ff9a;border:1px solid rgba(89,255,154,.25);padding:9px 11px;border-radius:8px}.miners-tools select{background:#070b16;color:#d9e7ff;border:1px solid rgba(100,210,255,.22);border-radius:8px;padding:9px 12px;font:700 10px JetBrains Mono,monospace}.miner-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:12px}.miner-card{position:relative;border:1px solid rgba(90,210,255,.14);background:linear-gradient(145deg,rgba(9,15,29,.92),rgba(4,7,15,.82));border-radius:12px;padding:16px;overflow:hidden;box-shadow:inset 0 0 35px rgba(70,80,180,.025)}.miner-card.active{border-color:rgba(89,255,154,.28);box-shadow:0 0 22px rgba(89,255,154,.06),inset 0 0 35px rgba(70,80,180,.03)}.miner-top{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.miner-name{font:700 13px JetBrains Mono,monospace;color:#e8f2ff;word-break:break-all}.miner-meta{margin-top:5px;font:9px JetBrains Mono,monospace;color:#70809a;letter-spacing:.06em}.miner-status{font:700 9px JetBrains Mono,monospace;color:#ff5577;border:1px solid rgba(255,85,119,.2);padding:5px 7px;border-radius:6px}.miner-status.on{color:#59ff9a;border-color:rgba(89,255,154,.25)}.miner-hash{margin:17px 0 13px;font:700 25px JetBrains Mono,monospace;color:#63f4ff;text-shadow:0 0 16px rgba(53,244,255,.22)}.miner-hash small{font-size:9px;color:#66758e;letter-spacing:.1em}.miner-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.miner-stat{border:1px solid rgba(100,120,160,.12);border-radius:8px;padding:9px}.miner-stat span{display:block;font:8px JetBrains Mono,monospace;color:#65738a;text-transform:uppercase;letter-spacing:.08em}.miner-stat strong{display:block;margin-top:5px;font:700 12px JetBrains Mono,monospace;color:#dce8fa}.miner-stat.ok strong{color:#59ff9a}.miner-stat.bad strong{color:#ff5577}.miner-bottom{display:flex;justify-content:space-between;gap:10px;margin-top:13px;font:8px JetBrains Mono,monospace;color:#65738a}.miner-bottom b{color:#c8a8ff}.miner-empty{padding:30px;text-align:center;color:#65738a;font:10px JetBrains Mono,monospace;border:1px dashed rgba(100,120,160,.16);border-radius:10px}@media(max-width:700px){.miners-head{align-items:flex-start;flex-direction:column}.miners-tools{width:100%}.miners-tools select{flex:1}.miner-grid{grid-template-columns:1fr}.miner-stats{grid-template-columns:repeat(2,1fr)}}''')

JS.write_text(JS.read_text())
print("patched dashboard: per-miner telemetry API + Armory UI")
''')

# Write a separate JS file rather than touching the large dashboard controller.
(ROOT / "monitor" / "static" / "dashboard_miner_stats.js").write_text(r'''(()=>{
'use strict';
...
const oldFetch=window.fetch;window.fetch=async function(...args){const r=await oldFetch.apply(this,args);try{const url=String(args[0]||'');if(url.includes('/api/status')){const clone=r.clone();const data=await clone.json();cache=Array.isArray(data.miners)?data.miners:[];render()}}catch(_){}return r};
})();
''')
print("created dashboard_miner_stats.js")
